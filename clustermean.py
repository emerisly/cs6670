...
def find_top_K_vectors(clusters_of_images, query_vector, K):
    ...
    cluster_means = clusters_of_images.mean(dim=1)  # Calculate the mean for each cluster
    query_vector_flat = query_vector.view(1, -1)  # Flatten the query vector
    cos_sim = torch.nn.functional.cosine_similarity(cluster_means.view(-1, rows*cols*channels), query_vector_flat, dim=1)
    ...
    for i in range(K):
        ...
        cos_sim_cluster = torch.nn.functional.cosine_similarity(clusters_of_images[cluster_index].view(-1, rows*cols*channels), query_vector_flat, dim=1)
        
        # Find the closest vector within the cluster
        closest_vector_index = torch.argmax(cos_sim_cluster)
        top_k_vectors.append(clusters_of_images[cluster_index].view(-1, rows*cols*channels)[closest_vector_index].numpy())
...
def find_top_K_C_vectors(clusters_of_images, query_vector, K, C=1):
    ...
    cluster_means = clusters_of_images.mean(dim=1)  # Calculate the mean for each cluster
    query_vector_flat = query_vector.view(1, -1)  # Flatten the query vector
    cos_sim = torch.nn.functional.cosine_similarity(cluster_means.view(-1, channels*rows*cols), query_vector_flat, dim=1)
    ...
    for i in range(K):
        ...
        # Calculate cosine similarity between the cluster vectors and the query vector
        cos_sim_cluster = torch.nn.functional.cosine_similarity(clusters_of_images[cluster_index].view(-1, channels*rows*cols), query_vector_flat, dim=1)
        ...
        top_C_vectors = clusters_of_images[cluster_index][top_C_indices].numpy().reshape(C, channels, rows, cols)
...
def calculate_and_print_cosine_similarity(clusters_of_images, query_vector, K, top_k_vectors):
    """
    ...
    for i in range(K):
        ...
        similarity = np.dot(query_vector.flatten(), top_K_vector_flat) / (np.linalg.norm(query_vector.flatten()) * np.linalg.norm(top_K_vector_flat))
    ...
    for cluster in clusters_of_images:
        for image in cluster:
            ...
            similarity = np.dot(query_vector.flatten(), image_flat) / (np.linalg.norm(query_vector.flatten()) * np.linalg.norm(image_flat))
    ...
    print("Similarities between the top K vectors and the query vector (in descending order):")
...
def testing():
    ...
    height = 48
    width = 64
    clusters_of_images = np.random.rand(number_of_clusters, batch, channels, height, width)
    ...
    calculate_and_print_cosine_similarity(clusters_of_images, query_vector, K, top_k_vectors)

    print("\nTOP K * C RETRIEVAL")
    top_K_C_vectors = find_top_K_C_vectors(clusters_of_images, query_vector, K, 2)
    calculate_and_print_cosine_similarity(clusters_of_images, query_vector, K*2, top_K_C_vectors) 
